...
module.exports = function elasticSearchPlugin(schema, options){
  ...
  /**
   * Default alterMapping method.
   */
  schema.statics.alterMapping = function() {}
  ...
  schema.statics.createMapping = function(settings, cb) {
    ...
    createMappingIfNotPresent(esClient, indexName, typeName, schema, settings, this.alterMapping, cb);
  };

  /**
   * @param indexName String (optional)
   * @param typeName String (optional)
   * @param options Object (optional) An options has with the following optional keys:
   *   indexName String (optional)
   *   typeName String (optional)
   *   params Object (optional) A has of params accepted by esClient.index().
   * @param callback Function
   */
  schema.methods.index = function(options, cb){
    ...
    var model = this;
    options.params.id = model._id.toString();
    setIndexNameIfUnset(model.constructor.modelName);
    esClient.index(options.index || indexName, options.type || typeName, serialize(model, mapping), options.params, cb);
  }
...
function createMappingIfNotPresent(client, indexName, typeName, schema, settings, alterMapping, cb) {
  generator.generateMapping(schema, function(err, mapping) {
    var completeMapping = {};
    // Allow mapping to be altered.
